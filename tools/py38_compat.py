...
def _is_none_literal(expression: ast.expr) -> bool:
    return (isinstance(expression, ast.Constant) and expression.value is None) or (
        isinstance(expression, ast.Name) and expression.id == "None"
    )


def _looks_like_type_expression(expression: ast.expr) -> bool:
    if isinstance(expression, (ast.Attribute, ast.Constant, ast.Name, ast.Subscript)):
        return True
    if isinstance(expression, ast.Tuple):
        return all(_looks_like_type_expression(item) for item in expression.elts)
...
class _TypeExpressionRewriter(ast.NodeTransformer):
    ...
    def _typing_subscript(self, name: str, expression: ast.expr) -> ast.Subscript:
...
class _ModuleTransformer(ast.NodeTransformer):
    ...
    def _rewrite_type_expression(
        self, expression: Optional[ast.expr]
    ) -> Optional[ast.expr]:
    ...
    def _visit_arguments(self, arguments: ast.arguments) -> ast.arguments:
        for argument in arguments.posonlyargs:
            argument.annotation = self._rewrite_type_expression(argument.annotation)
    ...
    def _ensure_future_import(self, body: List[ast.stmt]) -> List[ast.stmt]:
        ...
        return body[:insertion_index] + [future_import] + body[insertion_index:]
    ...
    def _ensure_typing_import(self, body: List[ast.stmt]) -> List[ast.stmt]:
        ...
        for statement in body:
            ...
            if missing_names:
                statement.names.extend(ast.alias(name=name) for name in missing_names)
            return body
        insertion_index = 0
        if body and _is_docstring(body[0]):
            insertion_index = 1
        ...
        return body[:insertion_index] + [typing_import] + body[insertion_index:]
    ...
    def visit_Assign(self, node: ast.Assign) -> ast.Assign:
        ...
        if _looks_like_type_expression(node.value):
            node.value = cast(ast.expr, self._rewrite_type_expression(node.value))
        return node

    def visit_AnnAssign(self, node: ast.AnnAssign) -> ast.AnnAssign:
        node.target = self.visit(node.target)
        node.annotation = cast(ast.expr, self._rewrite_type_expression(node.annotation))
    ...
    def visit_AsyncFunctionDef(
        ...
        node.decorator_list = [
            self._rewrite_decorator(decorator) for decorator in node.decorator_list
        ]
    ...
    def visit_ClassDef(self, node: ast.ClassDef) -> Union[ast.ClassDef, List[ast.stmt]]:
        ...
        if typevar_names:
            ...
            if len(typevar_names) == 1:
                slice_expression = ast.Name(id=typevar_names[0], ctx=ast.Load())
            else:
                slice_expression = ast.Tuple(
                    elts=[ast.Name(id=name, ctx=ast.Load()) for name in typevar_names],
                    ctx=ast.Load(),
                )
            node.bases.append(
                ast.Subscript(
